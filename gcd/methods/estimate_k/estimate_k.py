...
data = skewnorm.rvs(3, loc=50, scale=50, size=1000).astype(np.int)
...
DUMMY_ACCS = torch.from_numpy(y).repeat_interleave(int(MAX_K / 500)) * 100
...
def scipy_optimise(merge_test_loader, args):
    ...
    def dummy_func(K, args=None, dummy=None):
        if args != 0:
        ...
        return -DUMMY_ACCS[int(K)]

    # dummy_func_p = partial(dummy_func, args=0, dummy=0)
    # res = minimize_scalar(dummy_func_p, bounds=(10, 1000), method='bounded', options={'disp': True})
    # # res = minimize_scalar(dummy_func_p, method='brent')
    # print(res.x)
