...
def split_cluster_acc_v2(y_true, y_pred, mask, return_ind_map=False):
    """
    ...
    ind_map = {j: i for i, j in ind} # map GT labels to clusters
...
def split_cluster_acc_v3(y_true, y_pred, mask):
    """
    Calculate clustering accuracy. Require scikit-learn installed

    Enforce Old class mappings between predictions and Ground Truth are deterministic.
    Only do linear assignment between the New classes
    ...
    # Old class map
    ind_1 = np.array([[i, i] for i in old_classes_gt])

    # New class map
    ind_2 = linear_assignment(-w[len(old_classes_gt):len(w), len(old_classes_gt):len(w)])
    ind_2 += len(old_classes_gt)
